...
class WorkService {
  ...
  async create(workData) {
    try {
      ...
      const workPerAppointment = await Work.find({
        ...
        status: "done", // On ne compte que les travaux terminés
      });
      ...
      if (appointment.services.length === workPerAppointment.length) {
        ...
        console.log(
          `Services restants: ${appointment.services.length - workPerAppointment.length}`,
        ); // Pour le debug
      }
  ...
  async getByAppointmentId(id) {
    try {
      const work = await Work.find({ id_appointment: id }).populate("id_user");
      if (!work) {
        throw new Error("Travail non trouvé");
      }
      return work;
  ...
  async getByEmployee(employeeId) {
    try {
      return await Work.find({ id_user: employeeId })
        .populate("id_appointment")
        .populate("id_user")
        .sort({ date_start: -1 });
    } catch (error) {
